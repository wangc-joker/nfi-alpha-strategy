...
$targetConfig = Join-Path $FtUserDataRoot "user_data\config.backtest.alpha.top30.300u.json"
$sourceConfig = Join-Path $repoRoot "configs\config.backtest.alpha.top30.300u.json"
...
docker compose run --rm freqtrade backtesting `
    --config /freqtrade/user_data/config.backtest.alpha.top30.300u.json `
