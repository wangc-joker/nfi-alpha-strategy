"""Scalp long/short exit mode orchestration extracted from NFI."""
...
def short_exit_scalp(
    ...
    sell, signal_name = strategy.short_exit_signals(
      strategy.short_scalp_mode_name,
      profit_init_ratio,
      max_profit,
      max_loss,
      last_candle,
    ...
    if not sell:
      if is_system_v3_2:
        ...
        if strategy.system_v3_2_stops_enable and (
          ...
          sell, signal_name = True, f"exit_{strategy.short_scalp_mode_name}_stoploss_doom"
      elif is_system_v3_1:
        # Stoplosses
        if profit_stake < -(
          filled_entries[0].cost
    ...
    if strategy.target_profit_cache is not None and pair in strategy.target_profit_cache.data:
      ...
      if sell_max and signal_name_max is not None:
        return True, f"{signal_name_max}_m"
      if previous_sell_reason in [f"exit_{strategy.short_scalp_mode_name}_stoploss_u_e"]:
        if profit_ratio > (previous_profit + 0.005):
          mark_pair, mark_signal = strategy.mark_profit_target(
            strategy.short_scalp_mode_name,
            pair,
        ...
        previous_sell_reason not in [f"exit_{strategy.short_scalp_mode_name}_stoploss_doom"]
      ):
        # Update the target, raise it.
        mark_pair, mark_signal = strategy.mark_profit_target(
          strategy.short_scalp_mode_name,
    ...
    if sell and signal_name is not None:
      ...
      if strategy.target_profit_cache is not None and pair in strategy.target_profit_cache.data:
        previous_profit = strategy.target_profit_cache.data[pair]["profit"]
      if signal_name in [
        f"exit_{strategy.short_scalp_mode_name}_stoploss_doom",
        f"exit_{strategy.short_scalp_mode_name}_stoploss_u_e",
      ]:
        mark_pair, mark_signal = strategy.mark_profit_target(
          strategy.short_scalp_mode_name,
          pair,
      ...
      if profit_init_ratio >= 0.005:
        ...
        if strategy.target_profit_cache is not None and pair in strategy.target_profit_cache.data:
          previous_profit = strategy.target_profit_cache.data[pair]["profit"]
        if (previous_profit is None) or (previous_profit < profit_init_ratio):
          mark_signal = f"exit_profit_{strategy.short_scalp_mode_name}_max"
          strategy._set_profit_target(pair, mark_signal, current_rate, profit_init_ratio, current_time)

    if signal_name not in [
      f"exit_profit_{strategy.short_scalp_mode_name}_max",
      f"exit_{strategy.short_scalp_mode_name}_stoploss_doom",
      f"exit_{strategy.short_scalp_mode_name}_stoploss_u_e",
    ]:
      if sell and (signal_name is not None):
